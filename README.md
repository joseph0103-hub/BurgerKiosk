...
![메뉴 선택](img/Assignment_1_menuSelect.png)
![옵션 메뉴 선택](img/Assignment_1_optionSelect.png)
![주문 버튼](img/Assignment_1_orderBtn.png)
...
- 메뉴를 선택하지 않은 상태에서 Enter 키로 주문을 시도하면 동일하게 라벨에 경고 문구가 표시되도록 유지하였다.
...
![메뉴와 옵션메뉴 자동 추가1](img/Assignment_4_live_menu1.png)
![메뉴와 옵션메뉴 자동 추가2](img/Assignment_4_live_menu2.png)
![주문금액 자동 계산](img/Assignment_4_live_total.png)
![초기화 버튼](img/Assignment_4_reset.png)

### 과제 내용
- RadioButton과 CheckBox에서 원하는 항목을 선택하면 그 즉시 정보들이 업데이트되도록 구현한다.  
- 선택하는 순간 ListBox에 주문내역이 표시되도록 구현한다.  
- 선택하는 순간 Label에 전체 가격정보가 표시되도록 구현한다.  

### 구현 내용과 기능 설명
- RadioButton과 CheckBox에 CheckedChanged 이벤트를 연결하였다.  
- 메뉴나 옵션의 선택 상태가 바뀌는 즉시 주문 내역을 다시 계산하여 ListBox에 반영하도록 구현하였다.  
- 선택된 항목들의 금액을 즉시 합산하여 총 금액 Label에 실시간으로 표시되도록 구성하였다.  
- 메뉴가 선택되지 않은 상태에서 옵션만 선택된 경우에는 주문 내역을 비우고 총 금액을 0원으로 유지하면서 라벨 경고 문구를 표시하도록 처리하였다.  
- 주문하기 버튼을 누르지 않아도 화면 정보가 즉시 바뀌도록 구조를 개선하였으며, 키보드 이동과 Enter 주문 기능도 그대로 유지하였다.
